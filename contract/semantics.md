...
報價有**兩種**無效情形，來源不同，處理方式也不同。**兩者適用於每一種資料點，
bar 也不例外** —— `bid`/`ask` 在每一個 frame 上都送。

> 這句以前寫的是「兩者都只適用於 tick —— bar 不帶報價」。那是 tick/bar 分家年代的
> 規則，proto 2 合併 frame 形狀時就已經不成立，但這行沒有跟著改。實測（2026-08-03，
> SPY 全部 6 種圖表型態、6,405 列）**每一列 bar 都帶 `bid`/`ask`**。
>
> 當初不送的理由是「即時報價函式描述的是呼叫當下那一刻，不是整根 bar」—— 那句話
> 本身是對的，但它是一個**關於數字意義的意見**，而這條傳輸鏈不做那種判斷。同樣的
> 理由也拿掉了硬編碼的 index symbol 清單（§3.2 的歷史說明）。
