...
> 上表是**單一 publisher** 的情形。多個 TradeStation 程序同時發布時，同一條 topic 上會有
> 多條各自獨立的序號流，靠 `sid` 分辨 —— 完整的鍵是 `(sid, symbol)`，見 §6.3。
...
### 6.3 序號的鍵是 `(sid, topic)`，不是 `topic`

`sid` 不同代表某個 publisher 重啟、它的計數器歸零。此時必須清空**那一條流**的狀態，
否則會把「重啟」誤報成數十億筆遺漏。

**但清空的單位是 `(sid, topic)`，不是全部。** 這一條是強制的，而且漏做是靜默的：

TradeStation 10 每開一張圖就是一個新的 `orchart.exe` 程序，每個程序各有一份 DLL 狀態、
各自產生自己的 `sid`。所以 subscriber 的同一條連線上會有 **N 個 `sid` 同時在線**，逐 frame
交錯。若把 `sid` 存成單一純量、一發現不同就清空全部期望值：

- 每一根 bar 都會被判定成「publisher 重啟」；
- `_expected` 每次都被清空，於是每一筆都在走 §6.2 的「建立基準」；
- **永遠不會報告任何 gap**，`messages_lost` 恆讀 `0`。

資料照收、照存、內容全對，只有偵測死了 —— 而 `messages_lost == 0` 依 §6.6 的定義是
「沒有遺失」，於是一整個交易日會被歸檔成「已驗證完整」。這與 `frames_refused` 要防的是
同一類陷阱，差別在這次連拒收計數都不會動。

因此：

- 期望值以 `(sid, topic)` 為鍵，**各自獨立**；
- **不得因為看到另一個 `sid` 就丟棄既有的期望值。** 見下面〈控制 topic 永遠有 N 個 sid〉；
- 某個 `(sid, topic)` 第一次出現走 §6.2，建立基準、不報遺漏；
- 被取代的 `sid` 那一筆就讓它留著。它再也不會被比對到，一個整數的成本遠低於誤判的成本。

#### 控制 topic 永遠有 N 個 `sid` 同時在線

`__ts2py__` 是**固定的單一 topic**，每一個 chart 程序的 hello 都發在上面，各自帶自己的
`sid`，交錯抵達。所以它不是「偶爾會有兩個 sid」，而是**穩態就是 N 個**。

這使得「同一個 topic 換了 sid 就丟棄舊期望值」這種寫法在控制 topic 上等於原本的 bug：

```
hello sid=A seq=1  ->  expected[(A, ctrl)] = 2
hello sid=B seq=1  ->  丟棄 (A, ctrl)；expected[(B, ctrl)] = 2
hello sid=A seq=2  ->  丟棄 (B, ctrl)；(A, ctrl) 已經沒了 -> 重新建立基準
```

每一筆都在建立基準，控制 topic 的遺漏永遠偵測不到。**期望值必須各自獨立地留著。**

同樣的情形也發生在任何被兩個程序同時發布的 symbol topic —— 就是下一節那個「雙份」的案例。

#### 「新的 `sid` 出現」不等於「publisher 重啟了」

兩者在 wire 上**無法分辨**：`sid` 只說「這是某個 publisher session」，不說它是誰、也不說
舊的那個是否還活著。所以 binding 的記錄只能陳述可觀察的事實 ——「這個 topic 上出現了一個
先前沒見過的 publisher session」—— 並同時點名兩種成因：某個程序重啟了，或者又多了一個
程序在發同一條 topic。宣稱其中一種就是在猜。

這則記錄**每個 `(sid, topic)` 只發一次**。在交錯的串流上逐 frame 記錄，會把穩態當成事件。
...
`(symbol, bar_type, bar_interval)` 若被一個該圖先前沒出現過的 `sid` 宣告，有**兩種成因，
而 wire 分辨不出是哪一種** —— 與上一節對「新的 `sid` 出現」的判準完全相同：

| 成因 | 有沒有重複 |
|---|---|
| 該圖所屬的程序重啟了（TradeStation 重開、當機、Windows 更新重開機） | **沒有**。舊的那個已經死了，只是換了一個 `sid` |
| 另一個 TradeStation 程序也開了同一張圖 | **有**。兩邊都發布、topic 相同，該 topic 的每一筆都到兩次，而且兩條 `seq` 各自連續，沒有任何 gap 會揭露它 |

binding **必須**在偵測到時警告，並列出該圖已知的所有 `sid`。但**不得宣稱是哪一種成因** ——
TradeStation 一重啟，工作區裡每一張圖都會走到這條路徑，若訊息寫死成「資料重複」，
operator 會在一次尋常的重開機後收到 N 則假警報，然後學會忽略它，包括真的重複那一次。

這是 wire 上唯一能看見這件事的地方，所以訊息只能陳述可觀察的事實，把判斷留給讀的人。
